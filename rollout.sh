...
LEVEL_COMPRESS="${17}"
USER_DIR_TPCDS="${18}"
echo "show rollout RUN_COMPRESS - $RUN_COMPRESS, LEVEL_COMPRESS - $LEVEL_COMPRESS, USER_DIR_TPCDS - $USER_DIR_TPCDS"
...
echo "RUN_COMPRESS: $RUN_COMPRESS"
echo "LEVEL_COMPRESS: $LEVEL_COMPRESS"
echo "############################################################################"
echo ""
...
for i in $(ls -d $PWD/0*); do
	echo "$i/rollout.sh"
	$i/rollout.sh $GEN_DATA_SCALE $EXPLAIN_ANALYZE $RANDOM_DISTRIBUTION $MULTI_USER_COUNT $SINGLE_USER_ITERATIONS $RUN_OS $RUN_COMPRESS $LEVEL_COMPRESS
done
