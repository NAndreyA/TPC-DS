...
echo "TYPE_ORIENTATION: $TYPE_ORIENTATION"
echo "TYPE_COMPRESS: $TYPE_COMPRESS"
...
for i in $(ls -d $PWD/0*); do
	echo "$i/rollout.sh"
	$i/rollout.sh $GEN_DATA_SCALE $EXPLAIN_ANALYZE $RANDOM_DISTRIBUTION $MULTI_USER_COUNT $SINGLE_USER_ITERATIONS $TYPE_COMPRESS $LEVEL_COMPRESS $TYPE_ORIENTATION
done
